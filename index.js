...
const corsOptions = {
  origin: [
    "http://localhost:5173",
    "http://localhost:5174",
    "https://stay-finder-client.vercel.app",
  ],
  credentials: true,
...
async function run() {
  try {
    ...
    const bookingsCollection = client.db("stayFinderDb").collection("bookings");
    ...
    app.post("/bookings", verifyToken, async (req, res) => {
      ...
      if (result.insertedId) {
        ...
        sendEmail(booking.guest.email, {
          ...
          message: `Room Ready,  Transaction Id: ${booking.transactionId}`,
        });

        // To Host
        sendEmail(booking.host, {
          subject: "Your room got booked!",
          message: `Room booked by ${booking.guest.name} `,
        });
      }
      res.send(result);
    });
